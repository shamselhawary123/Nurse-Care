...
const handleAction = async (requestId: string, action: 'accept' | 'reject') => {
  try {
    const token = localStorage.getItem('token');
    await axios.patch(`/api/v1/request/${requestId}/${action}`, {}, {
      headers: { Authorization: `Bearer ${token}` },
    });
...
onMounted(() => {
  ...
  notificationSocket = io('https://health-care-tau-two.vercel.app/notifications');
  if (nurseId) {
    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <div v-else>
        ...
        <div v-for="req in requests" :key="req._id" class="request-card bg-white rounded-lg shadow p-6 mb-6 flex items-center gap-6">
          <img :src="req.patient.personalPhoto || '/default-avatar.png'" :alt="req.patient.name" class="w-16 h-16 rounded-full object-cover border" />
          <div class="flex-1">
            <div class="font-bold text-lg text-primary mb-2">{{ req.patient.name }}</div>
            <textarea class="input-field" :value="req.description" readonly rows="2"></textarea>
          </div>
          <div class="flex flex-col gap-2">
            <button class="accept-btn" @click="handleAction(req._id, 'accept')">قبول</button>
            <button class="reject-btn" @click="handleAction(req._id, 'reject')">رفض</button>
          </div>
        </div>
      </div>
    </div>
